...
#
# $Revision: 98105 $ $Date: 2009-01-20 04:34:32 -0500 (Tue, 20 Jan 2009) $
# 

import getopt, os.path, sys, xml.dom.minidom, binascii
...
def extractReplayLog(replayLogFileName, outputFileNamePrefix='./replay_'):
	...
	"""
	global eventIdx, eventFile, replayFiles, outputFilePrefix, lastSend
...
class PysysHandler(BaseHandler):
	def getOutputDirectory(self, outputdir):
		r = os.system('pysys make %s' % outputdir)
		if r != 0:
	...
	def processFiles(self, replayFiles):
			...
			pythonScriptFile.write("\t\tcorrelator.start(logfile='correlator.log', java=TRUE, Xclock=TRUE, arguments=['-XignoreEnqueue','-XsetRandomSeed','%d'])\n\n" % seed)
	...
	def processFiles(self, replayFiles):
		...
		if not testFramework:
			...
			if version>=3 and replayMode:
				argFlag = "--replayMode"
			pythonScriptFile.write("\t\tself.startCorrelator(Xclock=TRUE, args='%s -XsetRandomSeed %d', verbosity='INFO', abortOnError=TRUE, java=%s)\n\n" % (argFlag, seed, javaFlag))
...
def main():
	...
	global outputFileNamePrefix, outputDirectory, verbose, language, testFramework, javaEnabled, profile, replayMode

	try:
		opts, args = getopt.getopt(sys.argv[1:], "ho:l:tvpn", ["help", "output=", "verbose", "lang=", "testFramework", "profile", "noReplayMode"])
