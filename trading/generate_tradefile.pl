...
while ($date != $today){
    ...
    if (-e "/var/jsi/apama/logfiles/apamalogs_${date}.tar.bz2") {
        ...
        system("cat trades_${date}.csv corrections_${date}.csv | grep -v HTC | perl /var/jsi/tradefile_pnl/calculate_pnl.pl > pnl_${date}.csv");
        $array[$count] = "/var/jsi/atalogs/*correlator_${date}* ";
        $array2[$count] = "/var/jsi/atalogs/corrections_${date}* ";
        $count = $count + 1;
        system("cat @array @array2 | grep OrderUpdate | perl process_fills.pl | perl calculate_pnl.pl > cumlpnl_${date}.csv");
