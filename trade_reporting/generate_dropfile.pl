...
foreach my $venue (sort keys %positions) {
    ...
    foreach my $symbol (sort keys %{ $positions{"$venue"} }) {
        foreach my $side (sort keys %{ $positions{"$venue"}{"$symbol"} }) {
            ...
            if($venue eq "US"){
                print FILE "$count,2SDAAYF,,,XY,$shares,$avg_price,$side,P,$symbol,,,$date,,0.00,,,,,,,N,,,,,,4PEC83D,4PAA,T,0,,,,,,,,N,\n";
            } elsif ($venue eq "CA"){
                print FILE "$count,2SDAAYE,,,T,$shares,$avg_price,$side,P,$symbol,,,$date,,0.00,,,,,,,,,,,,,,,,,,,,,,\n";    
            }
